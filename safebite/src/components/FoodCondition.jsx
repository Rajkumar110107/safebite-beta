...
export default function FoodCondition({ onNavigate }) {
  ...
  const getConditionBadge = (cond) => {
    if (cond === 'SPOILED') return { label: 'ALERT / SPOILED', color: 'bg-rose-600 text-white', icon: 'dangerous', text: 'text-rose-600 dark:text-rose-400', ring: 'ring-rose-400' };
    if (cond === 'CONSUME_SOON') return { label: 'CONSUME SOON', color: 'bg-amber-500 text-white', icon: 'warning', text: 'text-amber-600 dark:text-amber-400', ring: 'ring-amber-400' };
    return { label: 'FRESH', color: 'bg-emerald-600 text-white', icon: 'check_circle', text: 'text-emerald-600 dark:text-emerald-400', ring: 'ring-emerald-400' };
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-6 border-b border-slate-200 dark:border-slate-800">
        <div>
          <div className="flex items-center gap-2.5 mb-1.5">
            <span className="p-1.5 sm:p-2 rounded-2xl bg-teal-100 dark:bg-teal-950/80 text-teal-800 dark:text-teal-300 border border-teal-200 dark:border-teal-800 flex items-center justify-center">
              <span className="material-symbols-outlined text-xl sm:text-2xl">health_and_safety</span>
            </span>
            <span className="text-[11px] sm:text-xs font-black uppercase tracking-widest text-teal-800 dark:text-teal-400 font-headline">
        ...
        <button
          ...
          className="w-full sm:w-auto px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 shadow-sm cursor-pointer transition-all active:scale-95 shrink-0"
        >
      ...
      {assessment ? (
        <div className="space-y-6 sm:space-y-8">
          ...
          <div className="bg-white dark:bg-[#131c2e] p-5 sm:p-8 rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-sm space-y-6 sm:space-y-8 transition-colors">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-6 pb-6 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-4">
                <span className="text-4xl sm:text-5xl p-3 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">{assessment.image || product.image}</span>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-400 font-mono">Scan Log: {assessment.id}</span>
                    <span className="text-[10px] font-mono text-slate-400 dark:text-slate-400">• {assessment.timestamp}</span>
                  </div>
                  <h2 className="text-xl sm:text-2xl md:text-3xl font-black font-headline text-slate-900 dark:text-white">{assessment.productName}</h2>
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Category: <strong className="text-slate-800 dark:text-slate-200">{assessment.category}</strong> • Batch: <strong className="text-slate-800 dark:text-slate-200">{assessment.batchId}</strong></p>
                </div>
              </div>

              <div className="flex items-center gap-3 self-start lg:self-auto">
                <div className={`px-4 sm:px-5 py-2 sm:py-2.5 rounded-full text-xs font-black uppercase tracking-wider flex items-center gap-2 shadow-md ${badge.color}`}>
                  <span className="material-symbols-outlined text-base sm:text-lg">{badge.icon}</span>
            ...
            <div className="space-y-3">
              <h3 className="font-headline font-black text-slate-900 dark:text-white text-sm sm:text-base flex items-center gap-2">
                <span className="material-symbols-outlined text-emerald-600 dark:text-emerald-400">compare_arrows</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="p-5 sm:p-6 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-3 transition-colors">
                  <div className="flex justify-between items-start">
                    <span className="text-[11px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-400 font-mono">Declared Label Expiry</span>
                    <span className="material-symbols-outlined text-slate-400">calendar_month</span>
                  </div>
                  <div className="space-y-1">
                    <p className="text-xl sm:text-2xl font-black font-headline text-slate-800 dark:text-slate-100">
                ...
                <div className={`p-5 sm:p-6 rounded-2xl border space-y-3 transition-colors ${
                  assessment.condition === 'SPOILED'
                    ? 'bg-rose-50 dark:bg-rose-950/30 border-rose-200 dark:border-rose-900/80'
                    : assessment.condition === 'CONSUME_SOON'
                  ...
                  <div className="flex justify-between items-start">
                    <span className="text-[11px] font-black uppercase tracking-wider font-mono text-slate-600 dark:text-slate-300">SafeBite Observed Screening</span>
                    <span className="material-symbols-outlined text-emerald-700 dark:text-emerald-400">sensors</span>
                  </div>
                  <div className="space-y-1">
                    <p className={`text-xl sm:text-2xl font-black font-headline ${badge.text}`}>
                      {badge.label}
                    </p>
            ...
            <div className="p-5 sm:p-6 rounded-2xl bg-slate-900 dark:bg-[#090d16] text-white space-y-5 border border-slate-800">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-800">
                ...
                <span className="text-[11px] font-mono text-emerald-400 bg-slate-800 px-2.5 py-1 rounded-full border border-slate-700 self-start sm:self-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="p-4 rounded-xl bg-slate-800/80 dark:bg-slate-900/80 border border-slate-700 space-y-1">
                ...
                <div className="p-4 rounded-xl bg-slate-800/80 dark:bg-slate-900/80 border border-slate-700 space-y-1">
                  ...
                  <div className="flex items-baseline gap-1">
                    ...
                    <span className="text-xs text-slate-400 ml-1">spoilage probability</span>
                  </div>
                </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-xs text-slate-600 dark:text-slate-300 space-y-1 transition-colors">
              <div className="flex items-center gap-2 font-bold text-slate-800 dark:text-slate-200">
                <span className="material-symbols-outlined text-slate-500 dark:text-slate-400 text-base">info</span>
        ...
        <div className="bg-white dark:bg-[#131c2e] p-8 sm:p-12 rounded-3xl border border-slate-200 dark:border-slate-800 text-center space-y-4 transition-colors">
          <span className="material-symbols-outlined text-5xl text-slate-300 dark:text-slate-600">qr_code_scanner</span>
          <h3 className="text-lg sm:text-xl font-bold font-headline text-slate-800 dark:text-white">No Active Screening Assessment</h3>
          ...
          <button
            onClick={() => onNavigate && onNavigate('Scan Food')}
            className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs transition-all cursor-pointer"
          >
            Start Scan
          </button>
        </div>

...
export default function History({ onNavigate }) {
  ...
  return (
    <div className="pt-20 sm:pt-24 px-4 sm:px-6 md:px-10 pb-16 max-w-7xl mx-auto space-y-6 sm:space-y-8 font-body transition-colors duration-200">
      ...
      {/* Scans Table with Horizontal Scrolling */}
      <div className="bg-white dark:bg-[#131c2e] rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
        <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30">
          <h3 className="font-headline font-black text-slate-900 dark:text-white text-sm sm:text-base">Screening Ledger</h3>
          <span className="text-xs font-mono text-slate-500 dark:text-slate-400">{filteredScans.length} records preserved</span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs min-w-[650px]">
            <thead className="bg-slate-50 dark:bg-slate-800/60 text-slate-500 dark:text-slate-400 font-mono uppercase text-[10px] tracking-wider border-b border-slate-200 dark:border-slate-700">
              <tr>
                <th className="px-5 sm:px-6 py-3 font-bold">Scan ID & Timestamp</th>
                <th className="px-5 sm:px-6 py-3 font-bold">Product & Category</th>
                <th className="px-5 sm:px-6 py-3 font-bold">Gas Telemetry</th>
                <th className="px-5 sm:px-6 py-3 font-bold">Screening Outcome</th>
                <th className="px-5 sm:px-6 py-3 font-bold">Condition Score</th>
                <th className="px-5 sm:px-6 py-3 font-bold text-right">Inspection Role</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-[#131c2e]">
              {filteredScans.length === 0 ? (
                <tr>
                  <td colSpan="6" className="px-6 py-12 text-center text-slate-400 dark:text-slate-400">
                ...
                filteredScans.map((scan) => (
                  <tr key={scan.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-5 sm:px-6 py-4">
                      <div className="font-mono font-bold text-slate-900 dark:text-white">{scan.id}</div>
                      <div className="text-[10px] text-slate-400 dark:text-slate-400 font-mono">{scan.timestamp}</div>
                    </td>
                    <td className="px-5 sm:px-6 py-4">
                      <span className="font-headline font-black text-slate-900 dark:text-white text-sm">{scan.productName}</span>
                      <div className="text-[10px] font-mono text-slate-400 dark:text-slate-400">{scan.category} • Batch: {scan.batchId}</div>
                    </td>
                    <td className="px-5 sm:px-6 py-4 font-mono">
                      <span className="font-bold text-slate-900 dark:text-white">{scan.gasValue} RAW</span>
                    ...
                    <td className="px-5 sm:px-6 py-4">
                      <div className="font-headline font-bold text-sm text-slate-900 dark:text-white">{scan.foodConditionScore}/100</div>
                      <div className="text-[10px] text-slate-400 font-mono">{scan.confidence} Conf.</div>
                    </td>
                    <td className="px-5 sm:px-6 py-4 text-right font-mono text-slate-500 dark:text-slate-400">

...
export default function AiAssistant() {
  const { latestScanAssessment } = usePlatform();
  ...
  // Helper to format messages and render **bold** syntax without raw asterisks
  const renderFormattedMessage = (text) => {
    const lines = text.split('\n');
    return lines.map((line, lineIdx) => {
      // Split on **bold text** tokens
...
• Store raw meats in leak-proof containers in the lowest chiller compartment.`;
  ...
  return (
    <div className="pt-20 sm:pt-24 px-4 sm:px-6 md:px-10 pb-16 max-w-5xl mx-auto space-y-6 sm:space-y-8 font-body transition-colors duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-6 border-b border-slate-200 dark:border-slate-800">
        ...
        {latestScanAssessment && (
          <div className="p-2.5 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/80 text-xs flex items-center gap-2 self-start sm:self-auto">
            <span>{latestScanAssessment.image || '🍱'}</span>
            <span className="font-bold text-slate-800 dark:text-slate-200">{latestScanAssessment.productName}:</span>
            <span className="font-black text-emerald-700 dark:text-emerald-400">{latestScanAssessment.statusText}</span>
          </div>
        )}
      </div>

      {/* Chat Container */}
      <div className="bg-white dark:bg-[#131c2e] rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col h-[520px] overflow-hidden transition-colors">
          ...
          {messages.map((m, idx) => (
            ...
            >
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold shrink-0 ${
                m.sender === 'user' ? 'bg-slate-900 dark:bg-emerald-700 text-white' : 'bg-emerald-600 text-white shadow-sm'
              }`}>
                {m.sender === 'user' ? 'You' : 'AI'}
              </div>
              <div className={`p-4 rounded-2xl text-xs leading-relaxed ${
                m.sender === 'user'
                  ? 'bg-slate-900 dark:bg-emerald-900/60 text-white rounded-tr-none'
                  : 'bg-slate-50 dark:bg-slate-800/80 border border-slate-200/90 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-tl-none'
              }`}>
        ...
        {/* Quick Prompts */}
        <div className="p-3 bg-slate-50/70 dark:bg-slate-800/40 border-t border-slate-100 dark:border-slate-800 flex gap-2 overflow-x-auto">
          {quickPrompts.map((prompt, i) => (
            <button
              key={i}
              onClick={() => handleSend(prompt)}
              className="px-3 py-1.5 rounded-xl bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700 text-[11px] font-medium text-slate-700 dark:text-slate-300 shrink-0 cursor-pointer transition-colors"
            >
              {prompt}
            </button>
          ))}
        ...
        {/* Query Input Box */}
        <div className="p-3 sm:p-4 bg-white dark:bg-[#131c2e] border-t border-slate-200 dark:border-slate-800 flex gap-2 transition-colors">
          <input
            type="text"
            value={inputQuery}
            ...
            className="flex-1 px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-emerald-500 font-medium transition-colors"
          />
          <button
            onClick={() => handleSend()}
            className="px-4 sm:px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 cursor-pointer shadow-sm transition-all active:scale-95 shrink-0"
          >
            <span>Ask</span>
      ...
      <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-xs text-slate-600 dark:text-slate-300 transition-colors">
        <strong className="text-slate-800 dark:text-slate-200">Safety Advisory:</strong> SafeBite AI provides culinary and preservation suggestions based on public food guidelines. It does not provide medical advice or substitute for laboratory testing. If in doubt regarding food spoilage or allergen safety, discard the item.
      </div>
